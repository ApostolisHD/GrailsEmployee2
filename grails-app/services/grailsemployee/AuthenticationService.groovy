...
class AuthenticationService {
    ...
    def getUserid(def user_name, def user_password) {
        def sql = new Sql(dataSource)
        return sql.firstRow("SELECT user_id FROM users where user_name=${user_name} AND user_password=${user_password}")
    }

    def logOut(def user_id) {
        def sql = new Sql(dataSource)
        sql.executeUpdate("UPDATE users SET user_active=${loggedIn} where user_id=${user_id}")
    }
